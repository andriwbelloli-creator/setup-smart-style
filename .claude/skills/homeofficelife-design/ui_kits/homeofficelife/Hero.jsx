// HomeOfficeLife · Hero — drop-zone overlay + headline + stats
/* eslint-disable */

const Hero = ({ onUpload, onExplore }) => {
  const [dragOver, setDragOver] = useState(false);

  const handleClick = () => onUpload?.("../../assets/hero-setup.webp");
  const handleDrop = (e) => {
    e.preventDefault();
    setDragOver(false);
    onUpload?.("../../assets/hero-setup.webp");
  };

  return (
    <section className="relative overflow-hidden"
             style={{ background: "var(--gradient-mesh), var(--background)" }}>
      <div className="mx-auto grid max-w-[1200px] gap-10 px-6 py-12 lg:grid-cols-[1.1fr_1fr] lg:gap-12 lg:py-16">
        ...
        <div className="flex flex-col justify-center" style={{ animation: "fadeUp 0.8s var(--ease-smooth) both" }}>
          <Pill tone="eyebrow" className="mb-5 w-fit !text-[11px]">
            ...
            <Icon.Zap size={13}/> IA brasileira · Diagnóstico em 30 segundos
          </Pill>

          <h1 style={{ fontFamily: "var(--font-display)" }}
              className="text-[44px] sm:text-[56px] lg:text-[64px] font-bold tracking-[-0.025em] leading-[1.02] text-[var(--foreground)]">
            Descubra a{" "}
          ...
          <p className="mt-5 max-w-xl text-[17px] leading-[1.55] text-[var(--muted-foreground)]">
            Envie a foto do seu home office e receba{" "}
            <strong className="text-[var(--foreground)]">nota de ergonomia, iluminação, organização</strong>{" "}
            + dicas com produtos reais do Brasil.
          </p>

          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <Button variant="hero" size="xl" onClick={handleClick}>
              <Icon.Upload size={18}/> Mandar foto e ver minha nota
            </Button>
            <Button variant="outline" size="xl" onClick={onExplore}>
              Explorar galeria <Icon.ArrowRight size={16}/>
            </Button>
          </div>

          <p className="mt-3 text-xs text-[var(--muted-foreground)]">
            <strong className="text-[var(--foreground)]">3 análises gratuitas</strong> — sem cartão, sem pegadinha. Resultado em ~30s.
          </p>
          ...
          <div className="mt-7 flex flex-wrap items-center gap-6 text-sm text-[var(--muted-foreground)]">
            ...
            <div className="flex items-center gap-1">
              {[...Array(5)].map((_, i) => (
                <Icon.Star key={i} size={15} className="text-[var(--brand-coral-500)]"/>
              ))}
        ...
        <div className="relative">
          <label
            ...
            onClick={handleClick}
            className={
              "group relative block cursor-pointer overflow-hidden rounded-[28px] " +
              "bg-[var(--card)] shadow-[var(--shadow-elegant)] transition-all duration-300 " +
              "[transition-timing-function:cubic-bezier(0.22,1,0.36,1)] " +
              (dragOver
                ? "border-4 border-[var(--primary)] scale-[1.02] shadow-[var(--shadow-glow)]"
                : "border-2 border-[var(--border)] hover:border-[var(--primary)]/50")
            }
          >
            <img src="../../assets/hero-setup.webp" alt="Setup home office wood + teal"
                 className="block aspect-[16/11] h-auto w-full object-cover transition-transform duration-300 group-hover:scale-105"/>
            ...
            <div className={
              ...
              <div className="rounded-full bg-[var(--background)]/95 p-5 shadow-[var(--shadow-elegant)] backdrop-blur">
                <Icon.Image size={36} className="text-[var(--primary)]"/>
              </div>
              ...
              <p className="max-w-[260px] px-4 text-center text-sm text-white/90">
            ...
            <div className="absolute bottom-3 right-3 rounded-xl bg-[var(--card)]/95 p-3 shadow-[var(--shadow-elegant)] backdrop-blur">
              ...
              <div className="mt-1 flex gap-0.5">
                {[1, 1, 1, 1, 0].map((on, i) => (
                  <span key={i} className={"h-1 w-4 rounded-full " + (on ? "bg-[var(--primary)]" : "bg-[var(--muted)]")}/>
                ))}
              </div>
            </div>
          </label>
          ...
          <p className="mt-3 text-center text-xs text-[var(--muted-foreground)]">
            <Icon.Sparkles size={11} className="inline-block mr-1 text-[var(--brand-coral-500)]"/>
            Arraste uma foto pra cima da imagem ou clique
          </p>

          <div className="pointer-events-none absolute -right-8 -top-8 -z-10 h-40 w-40 rounded-full opacity-30 blur-3xl"
               style={{ background: "var(--accent)" }}/>
          <div className="pointer-events-none absolute -bottom-8 -left-8 -z-10 h-40 w-40 rounded-full opacity-30 blur-3xl"
