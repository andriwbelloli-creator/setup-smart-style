// HomeOfficeLife · Análise IA — diagnostic results
/* eslint-disable */
...
const TIPS = [
  { title: "Iluminação fraca à esquerda", text: "Uma luminária de mesa LED com braço articulado (~R$ 180) eliminaria a sombra no teclado." },
  { title: "Monitor inclinado pra baixo",  text: "Levante o monitor 6–8cm com um suporte (~R$ 120). Topo da tela na linha dos olhos." },
  { title: "Excelente uso de plantas",     text: "Mantém o ar visualmente leve. Considere uma samambaia pra refrescar o canto direito." },
];

const PICKS = [
  { name: "Luminária Yeelight LED",  price: "R$ 169", store: "Amazon BR", impact: "+15pts iluminação" },
  { name: "Suporte VESA Multilaser", price: "R$ 119", store: "Mercado Livre", impact: "+12pts ergonomia" },
  { name: "Apoio lombar Flexform",   price: "R$ 249", store: "Amazon BR", impact: "+8pts conforto" },
];
...
const AnaliseIA = ({ uploadedSrc, onReset, onFullDiag }: AnaliseIAProps) => {
    ...
    <section id="diagnostico" className="border-y border-[var(--border)] bg-[var(--cream)] py-14">
      <div className="mx-auto max-w-[1200px] px-6">
        <div className="mb-8 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
          <div>
            <Pill tone="eyebrow" className="mb-3 !text-[10px]"><I.Sparkles size={11}/> Resultado IA</Pill>
            <h2 style={{ fontFamily: "var(--font-display)" }} className="text-[28px] sm:text-[36px] font-bold leading-tight tracking-[-0.025em] text-[var(--foreground)]">
              {stage === "analyzing" ? "Analisando seu setup…" : "Pronto — sua nota está aqui"}
            </h2>
          </div>
          <Button variant="ghost" size="sm" onClick={onReset}><I.X size={14}/> Nova foto</Button>
        </div>
              ...
              {stage === "analyzing" && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-[var(--foreground)]/50 backdrop-blur-[2px]">
                  <div className="rounded-full bg-white/95 p-4 shadow-[var(--shadow-elegant)]">
                    <I.Sparkles size={28} className="text-[var(--brand-coral-500)]" style={{ animation: "pulse 1.5s ease-in-out infinite" }}/>
                  </div>
                  <div className="w-[280px] max-w-[70%]">
                    <div className="h-1.5 overflow-hidden rounded-full bg-white/30">
                      <div className="h-full rounded-full bg-white transition-[width] duration-200" style={{ width: progress + "%" }}/>
                    </div>
              ...
              {stage === "done" && (
                <>
                  <div className="absolute right-3 top-3 rounded-xl bg-white/96 p-3 shadow-[var(--shadow-elegant)] backdrop-blur">
                    ...
                    <div style={{ fontFamily: "var(--font-display)" }} className="text-3xl font-bold leading-none text-[var(--foreground)]">
                      8.4<span className="text-base text-[var(--muted-foreground)]">/10</span>
                    </div>
                  </div>
                  <div className="absolute left-3 top-3"><Pill tone="coral"><I.Sparkles size={11}/> Top 18% Brasil</Pill></div>
                </>
              )}
            </div>
          </Card>
                ...
                <div key={a.label}>
                  ...
                  <div className="h-2 overflow-hidden rounded-full bg-[var(--muted)]">
                    <div className="h-full rounded-full transition-[width] duration-[1200ms]"
                         style={{ width: stage === "done" ? (a.value * 10) + "%" : "0%", background: a.tone, transitionTimingFunction: "cubic-bezier(0.22,1,0.36,1)" }}/>
                  </div>
                </div>
              ))}
            </div>
            {stage === "done" && (
              <Button variant="hero" className="mt-6 w-full !h-11" onClick={onFullDiag}>
                <I.Eye size={14}/> Ver diagnóstico completo
              </Button>
            )}
          </Card>
        </div>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {TIPS.map((t, i) => (
              <Card key={i} className="!rounded-[20px] p-5">
                ...
                <h4 style={{ fontFamily: "var(--font-display)" }} className="text-[15px] font-bold leading-tight text-[var(--foreground)]">{t.title}</h4>
          ...
          <Card className="mt-5 !rounded-[24px] p-6">
            <div className="mb-4 flex items-baseline justify-between">
              <div>
                ...
                <h3 style={{ fontFamily: "var(--font-display)" }} className="text-xl font-bold text-[var(--foreground)]">3 upgrades em ordem de impacto</h3>
              </div>
            </div>
            <div className="grid gap-3 md:grid-cols-3">
              {PICKS.map((p, i) => (
                <div key={i} className="flex items-center gap-3 rounded-2xl border border-[var(--border)] bg-[var(--background)] p-3 cursor-pointer hover:border-[var(--primary)]/50 transition-colors"
                     onClick={() => toast.show("Abrindo " + p.name + " na " + p.store)}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[11px] font-semibold text-[var(--success)]">{p.impact}</div>
                  </div>
                </div>
              ))}
            </div>
