...
// ── useTweaks ───────────────────────────────────────────────────────────────
export function useTweaks<T extends Record<string, any>>(defaults: T): [T, (keyOrEdits: string | Partial<T>, val?: any) => void] {
  ...
  const setTweak = useCallback((keyOrEdits: string | Partial<T>, val?: any) => {
    const edits: Partial<T> = typeof keyOrEdits === 'object' && keyOrEdits !== null
      ? keyOrEdits : { [keyOrEdits as string]: val } as Partial<T>;
...
export interface TweakRadioOption {
  value: any;
  label: string;
}
export interface TweakRadioProps {
  label: string;
  value: any;
  options: (string | TweakRadioOption)[];
  onChange: (v: any) => void;
}
export function TweakRadio({ label, value, options, onChange }: TweakRadioProps) {
  ...
  if (!fitsAsSegments) {
    const resolve = (s: string) => {
      const m = options.find((o) => String(typeof o === 'object' ? (o as TweakRadioOption).value : o) === s);
      return m === undefined ? s : typeof m === 'object' ? (m as TweakRadioOption).value : m;
    };
    return <TweakSelect label={label} value={value} options={options}
                        onChange={(s: string) => onChange(resolve(s))} />;
  }
  const opts = options.map((o) => (typeof o === 'object' ? o as TweakRadioOption : { value: o, label: o }));
  ...
  return (
    <TweakRow label={label}>
      <select className="twk-field" value={value} onChange={(e) => onChange(e.target.value)}>
        {options.map((o) => {
          const v = typeof o === 'object' ? (o as TweakRadioOption).value : o;
          const l = typeof o === 'object' ? (o as TweakRadioOption).label : o;
